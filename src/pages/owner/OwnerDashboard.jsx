...
const OwnerDashboard = () => {
  const { user } = useAuth();
  const [stats, setStats] = useState({ totalBikes: 0, totalBookings: 0, activeRentals: 0, earnings: 0, spent: 0 });
  ...
  useEffect(() => {
    const loadDashboard = async () => {
      ...
      try {
        ...
        const ownerBikes = allBikes.filter((bike) => String(bike.ownerId) === String(user.uid));
        const activeRentals = ownerBookings.filter((b) => b.status === "active");
        ...
        const spent = userBookings.reduce((sum, b) => sum + (Number(b.price) || 0), 0);
        setStats({ totalBikes: ownerBikes.length, totalBookings: userBookings.length, activeRentals: activeRentals.length, earnings, spent });
  ...
  return (
    <div className="bg-background min-h-screen p-4 sm:p-6 space-y-4 sm:space-y-6">
